...
class ReferenceTests(CheckerTestCase):
    ...
    def test_errors(self):
        cases = [
            ...
            ("dereference_record_before_payload", "*new({a = missing})", "Nat", "n : Nat", "ERROR_UNEXPECTED_RECORD"),
            ("dereference_tuple_before_payload", "*new({missing, 0})", "Nat", "n : Nat", "ERROR_UNEXPECTED_TUPLE"),
            ("dereference_list_before_payload", "*new([missing])", "Nat", "n : Nat", "ERROR_UNEXPECTED_LIST"),
            ("dereference_preserves_first_field_error", "*new({a = 0, b = missing})", "{a : Bool, b : Nat}", "n : Nat", "ERROR_UNEXPECTED_TYPE_FOR_EXPRESSION"),
